...
#ifndef DYNET_HOROVOD_MPI_OPS_H
#define DYNET_HOROVOD_MPI_OPS_H
...
struct HorovodGlobalState {
  ...
  std::unordered_map<int, dynet::Tensor*> tensor_fusion_buffers;
...
void BackgroundThreadLoop(HorovodGlobalState& state) {
  ...
  do {
    ...
    while (!message_queue.empty()) {
      ...
      if (is_coordinator) {
        bool reduce = IncrementTensorCount(state.message_table, message, size); //TODO
        if (reduce) {
    ...
    if (is_coordinator) {
      ...
      while (completed_ranks != size) {
        ...
        bool reduce =
            IncrementTensorCount(state.message_table, received_message, size); //TODO
        if (reduce) {
      ...
      for (auto it = ready_to_reduce.begin(); it != ready_to_reduce.end();
           it++) {
        MPIResponse response = ConstructMPIResponse(state.message_table, *it); //TODO
      ...
      while (!responses.empty()) {
        ...
        // the same operation.
        PerformOperation(state.tensor_table, response); //TODO
      }
...
extern "C" void horovod_tensorflow_init() {}
